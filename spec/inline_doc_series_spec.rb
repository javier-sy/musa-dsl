...
RSpec.describe 'Series Inline Documentation Examples' do
  ...
  context 'Timed Serie (timed-serie.rb)' do
    it 'example from line 28 - Timed union' do
    ...
    it 'example from line 34 - Flatten timed' do
      multi = S({time: 0r, value: {a: 60, b: 64}})
        .map { |v| v.extend(Musa::Datasets::AbsTimed) }
      flat = multi.flatten_timed
      result = flat.i.next_value
      expect(result).to be_a(Hash)
      expect(result[:a][:time]).to eq(0r)
      expect(result[:a][:value]).to eq(60)
    end

    it 'example from line 58 - Hash mode' do
      s1 = S({time: 0r, value: 1}, {time: 1r, value: 2})
        .map { |v| v.extend(Musa::Datasets::AbsTimed) }
      s2 = S({time: 0r, value: 10}, {time: 1r, value: 20})
        .map { |v| v.extend(Musa::Datasets::AbsTimed) }
      union = TIMED_UNION(melody: s1, bass: s2)
      inst = union.i
      first = inst.next_value
    ...
    it 'example from line 62 - Array mode' do
      s1 = S({time: 0r, value: 1}, {time: 1r, value: 2})
        .map { |v| v.extend(Musa::Datasets::AbsTimed) }
      s2 = S({time: 0r, value: 10}, {time: 1r, value: 20})
        .map { |v| v.extend(Musa::Datasets::AbsTimed) }
      result = TIMED_UNION(s1, s2)
      expect(result.i.to_a.size).to be > 0
    end
  end

  context 'Series Composer (series-composer.rb)' do
    it 'example from line 32 - Basic composer' do
      s = S(1, 2, 3).composer do
        route(:input, to: :output)
      end
      # Composer creates buffered series
      expect(s.i.to_a).to eq([1, 2, 3])
    end

    it 'example from line 61 - Simple transformation' do
      s = S(1, 2, 3).composer do
        route(:input, to: :output)
      end
      # Composer routes input through to output
      expect(s.i.to_a).to eq([1, 2, 3])
    end
  end
