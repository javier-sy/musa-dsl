...
RSpec.describe Musa::Neumalang do
  context 'Neuma simple parsing' do
    it 'Basic neuma inline parsing (only a comment)' do
      expect(Musa::Neumalang.parse('/* comentario 1 */').to_a(true)).to eq([])
    end

    it 'Basic neuma inline parsing (two comments)' do
      expect(Musa::Neumalang.parse('/* comentario 1 */ /* bla bla */').to_a(true)).to eq([])
    end

    it 'Basic neuma inline parsing (two comments with subcomments)' do
      expect(Musa::Neumalang.parse('/* comentario  /* otro comentario */ 1 */ /* bla bla */').to_a(true)).to eq([])
    end

    it 'Basic neuma inline parsing (1)' do
      expect(Musa::Neumalang.parse('2.3.4 5.6.7 /* comentario 1 */ ::evento').to_a(true)).to eq(
        [{ kind: :neuma, neuma: %w[2 3 4] }, { kind: :neuma, neuma: %w[5 6 7] }, { kind: :event, event: :evento }]
      )
    end

    it 'Basic neuma inline parsing (2)' do
      expect(Musa::Neumalang.parse('2.3.4 5.6.7 ::evento /* comentario 1 */').to_a(true)).to eq(
        [{ kind: :neuma, neuma: %w[2 3 4] }, { kind: :neuma, neuma: %w[5 6 7] }, { kind: :event, event: :evento }]
      )
    end

    it 'Basic neuma inline parsing with octaves' do
      expect(Musa::Neumalang.parse('2.o-1.3.4 5.o2.6.7 ::evento').to_a(true)).to eq(
    ...
    it 'Basic neuma file parsing with GDV decoder' do
      ...
      result = Musa::Neumalang.parse_file(File.join(File.dirname(__FILE__), 'neuma_spec.neu'), decode_with: decoder).to_a(true)
